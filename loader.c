...
void phdr_table(Elf32_Phdr * phdr, int arg) {
    char flags[3] = {'R', 'W', 'E'};
    for (int i = 0; i < 3; i++) if (!((phdr -> p_flags >> (2-i)) & 1)) flags[i] = ' ';
    printf("%-9d\t%x\t%x\t\t%x\t\t%x\t%x\t%s\t%x\n",
        phdr -> p_type,
        phdr -> p_offset,
        phdr -> p_vaddr,
        phdr -> p_paddr,
...
void load_phdr(Elf32_Phdr * phdr, int fd) {
    ...
    mmap( (void*)vaddr, phdr -> p_memsz + padding, permissions, MAP_PRIVATE | MAP_FIXED, fd, offset);
    phdr_table(phdr, 0);
}
